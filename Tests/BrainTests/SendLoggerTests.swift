...
@Suite struct SendLoggerTests {
    ...
    @Test func testCompleteIsLoggedWithPurposeAndBytes() async throws {
        let sink = Sink()
        ...
        #expect(r.text == "ok:hello")
        let rows = await sink.rows
        #expect(rows.count == 1)
        #expect(rows[0].0 == "Judge what matters"); #expect(rows[0].1 == "m1"); #expect(rows[0].3 == "3 summaries")
        #expect(rows[0].2 == "SYSTEM:\nsys\n\nINPUT:\nhello world".utf8.count, "bytes are the exact payload")
        let results = await sink.results
        #expect(results.first?.0 == 1); #expect(results.first?.1.contains("tokens") ?? false)
    }
    @Test func testPayloadIsCappedButSizeIsHonest() async throws {
        let sink = Sink()
        ...
        #expect(row.2 > SendLogger.payloadCap, "reported bytes are the real size")
        #expect(row.4.utf8.count < SendLogger.payloadCap + 200, "stored text is capped")
        #expect(row.4.contains("more bytes"))
    }
    @Test func testNonAgenticBrainStaysNonAgentic() {
        #expect(!(SendLogger.wrap(Echo(), model: "m", logger: SendLogger(sink: { _, _, _, _, _ in nil }, result: { _, _ in })).isAgentic))
    }
    @Test func testFormattedBytes() {
        #expect(512.formattedBytes == "512 B"); #expect(14_540.formattedBytes == "14.2 KB"); #expect(3_000_000.formattedBytes == "2.9 MB")
    }
}
