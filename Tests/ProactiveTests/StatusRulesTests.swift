...
@Suite struct StatusRulesTests {
    ...
    @Test func settledItemsAreShownFourteenDaysThenLeave() {
        ...
        #expect(!StatusRules.leftToday(settledAt: nil, now: now) && !StatusRules.leftToday(settledAt: now.addingTimeInterval(-13 * day), now: now))
        #expect(StatusRules.leftToday(settledAt: now.addingTimeInterval(-14.5 * day), now: now), "left the block since yesterday's run")
        #expect(!StatusRules.leftToday(settledAt: now.addingTimeInterval(-15 * day), now: now), "left before yesterday: already handed on")
