...
| Telegram app | my.telegram.org → API development tools | `TELEGRAM_API_ID`, `TELEGRAM_API_HASH` | Telegram source. Shipped in the bundle; a user can replace it with their own from Settings → Sources → Telegram → "Use your own app…" (kept in their Keychain, used from the next launch) |
