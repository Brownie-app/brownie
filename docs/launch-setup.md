...
| Slack app | api.slack.com/apps → OAuth & Permissions: user scopes `channels:history channels:read groups:history groups:read im:history im:read mpim:history mpim:read users:read`; redirect URL `https://brownie-website-dusky.vercel.app/oauth/slack` (the page bounces to `brownie://oauth/slack`) | `SLACK_CLIENT_ID`, `SLACK_CLIENT_SECRET` | Slack source (users can also paste a user token) |
