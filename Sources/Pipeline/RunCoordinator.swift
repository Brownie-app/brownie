...
public actor RunCoordinator {
    ...
    static func describe(_ a: Availability) -> String {
        ...
        case .needsPermission(let p):
            switch p {
            case .fullDiskAccess: return "needs Full Disk Access — macOS never asks for this one; add Brownie yourself in System Settings → Privacy & Security → Full Disk Access"
            case .calendar: return "needs Calendar — press Allow on the Calendar row in Settings → Sources"
            case .contacts: return "needs Contacts — press Allow on the Contacts row in Settings → Sources"
            case .speech: return "needs Speech Recognition — turn it on in System Settings → Privacy & Security"
            default: return "needs \(p.rawValue)"
            }
