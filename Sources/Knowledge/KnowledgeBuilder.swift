...
public actor KnowledgeBuilder {
    ...
    /// `people` is the registry's roster, so the brain knows which file each person already has. `selfNames` are the
    /// user's own, so the tools refuse a People or Groups note about them.
    public init(brain: any Brain, store: any KnowledgeStore, runStore: any RunStore, bundle: Bundle? = nil,
                ...
                coverage: @escaping @Sendable () async -> String? = { nil }, people: @escaping @Sendable () async -> [Person] = { [] }, selfNames: [String] = []) throws {
        let bundle = bundle ?? Bundle.module
        self.brain = brain; self.store = store; self.runStore = runStore
        self.partBudget = partBudget; self.now = now; self.timeZone = timeZone; self.coverage = coverage; self.people = people; self.selfNames = SelfNames.clean(selfNames)
