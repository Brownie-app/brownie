...
public enum StatusBlock {
    ...
    public static func render(person: String, asks: [Ask], loops: [Loop], now: Date, timeZone: TimeZone = .current) -> String {
        let lines = entries(person: person, asks: asks, loops: loops, timeZone: timeZone).filter { StatusRules.isShown(settledAt: $0.settledAt, now: now) }.map(\.line)
    ...
    public static func retiredLines(person: String, asks: [Ask], loops: [Loop], now: Date, timeZone: TimeZone = .current) -> [String] {
        entries(person: person, asks: asks, loops: loops, timeZone: timeZone).filter { StatusRules.leftToday(settledAt: $0.settledAt, now: now) }.map(\.line)
    }
    ...
    static func entries(person: String, asks: [Ask], loops: [Loop], timeZone: TimeZone) -> [Entry] {
        ...
        let theirAsks = asks.filter { AskLedger.samePerson($0.person, person) }.sorted { ($1.askedAt, $0.id) < ($0.askedAt, $1.id) }.prefix(8)
        let theirLoops = loops.filter { AskLedger.samePerson($0.person, person) && $0.status != .dismissed }
