    ...
    public func read(_ source: any Source, enabledBuckets: Set<BucketID>?, runID: Int64, mode: Mode = .auto,
        ...
        for (bi, bucket) in buckets.enumerated() {
            ...
            for (ii, item) in plan.items.enumerated() {
                ...
                // Mail carries no date until it is fetched: what the load knows widens what this source is known to cover.
                if let d = artifact.candidate.itemDate { oldest = min(oldest ?? d, d); newest = max(newest ?? d, d) }
