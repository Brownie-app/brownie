        ...
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text(permission == .fullDiskAccess ? "macOS never asks for this one. Drag this icon into the list on the right, then flip the switch on."
                         : "Drag this icon into the list on the right, then flip the switch on.").font(.system(size: 12)).foregroundStyle(.secondary)
                }
            }
            ...
            if permission == .fullDiskAccess {
                Text("Brownie already in the list and still not working? Select it, press −, then add it again: an entry made before Brownie was signed no longer matches this app.").font(.system(size: 11)).foregroundStyle(.secondary)
            }
            HStack(spacing: 8) {
                Button("Open the list") { PermissionProbe.openSettings(for: permission) }.font(.system(size: 12))
                Button("Show Brownie in Finder") { NSWorkspace.shared.activateFileViewerSelecting([Bundle.main.bundleURL]) }.font(.system(size: 12))
            }
