...
/// One source as the screen needs it: plain values, never `any Source`. SwiftUI's ForEach projects each element of
/// its collection out of a box on every layout pass, and an existential box is not a safe thing to hand it — the app
/// died on this screen doing exactly that. The descriptor's facts are copied once; the source itself stays in the model.
struct SourceRow: Identifiable, Equatable {
    ...
    let perBucket: Bool
    init(_ s: any Source) {
        id = s.id; name = s.descriptor.name; detail = s.descriptor.detail; perBucket = s.descriptor.supportsPerBucketOptIn
    }
}
...
struct SourcesPane: View {
    ...
    var body: some View {
        ...
        CardBox(padding: 0) {
            VStack(spacing: 0) {
                ForEach(personal) { s in
                    SettingRow(title: s.name, detail: detail(s)) {
        ...
        ForEach(m.sourceRows) { s in
            if s.perBucket, m.enabledSources.contains(s.id), let all = m.discovered[s.id], !all.isEmpty {
                ...
                HStack { Text(s.id == "files" ? "Folders" : "Chats · \(s.name)").font(.system(size: 14, weight: .semibold)); Spacer()
    ...
    var personal: [SourceRow] { m.sourceRows.filter { !Self.spokenIDs.contains($0.id) && !Self.workChatIDs.contains($0.id) } }
    var workChat: [SourceRow] { m.sourceRows.filter { Self.workChatIDs.contains($0.id) } }
    @State private var showSlackToken = false
    var spoken: [SourceRow] { m.sourceRows.filter { $0.id == "voicememos" || $0.id == "recordings" } }
    ...
    @ViewBuilder func firstReadLine(_ id: SourceID) -> some View {
        if m.enabledSources.contains(id), let line = FirstRead.current.describe(for: id) {
            VStack(alignment: .leading, spacing: 3) {
                ...
                // What it has actually read, so a source that is connected and simply quiet is never mistaken for a broken one.
                if let c = m.coverage[id], m.availability[id] == .available {
                    Text(CoverageLine.status(c, now: Date())).font(.system(size: 11)).foregroundStyle(t.ink3)
                }
            }.padding(.horizontal, 16).padding(.bottom, 8)
        }
    }
    func detail(_ s: SourceRow) -> String {
        ...
        case .notInstalled: return "\(s.name) isn't on this Mac"
        ...
        default:
            ...
            if s.perBucket { return "\(m.enabledBuckets[s.id]?.count ?? 0) \(Self.workChatIDs.contains(s.id) ? "conversations" : "chats") chosen" }
            return s.detail
        }
    }
    func chooseFolder() {
