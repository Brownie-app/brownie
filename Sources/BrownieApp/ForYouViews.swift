        ...
        VStack(spacing: 0) {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    if m.lostFullDiskAccess {
                        HStack(alignment: .top, spacing: 12) {
                            Image(systemName: "exclamationmark.triangle").foregroundStyle(t.warn)
                            VStack(alignment: .leading, spacing: 6) {
                                Text("Brownie can't read your chats any more").fontWeight(.semibold)
                                Text("Messages and Apple Notes were not read. macOS never asks for Full Disk Access — you add Brownie to the list yourself, and an entry made before Brownie was signed no longer matches, so remove it with − and add it again. Then press Analyse now.").foregroundStyle(t.ink2)
                                BButton(title: "Grant Full Disk Access again", kind: .primary) { PermissionGuide.shared.show(for: .fullDiskAccess) { Task { await m.refreshPermissions(); await m.refreshSources(); await m.reload() } } }
                            }
