        ...
        case 4:
            ...
            CardBox(padding: 0) { VStack(spacing: 0) {
                ForEach(m.sourceRows) { s in
                    SettingRow(title: s.name, detail: s.detail) { Toggle2(on: Binding(get: { m.enabledSources.contains(s.id) }, set: { _ in m.toggleSource(s.id) })) }.padding(.horizontal, 16); Divider()
                }
