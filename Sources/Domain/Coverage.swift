...
public enum CoverageLine {
    ...
    /// What this source has actually read, for the person looking at Settings → Sources: how much, how far back, when
    /// it last looked, and whether there was anything new. A source that is connected and simply quiet says so —
    /// "Gmail · nothing new since 19 Sep" — so it is never mistaken for one that is broken.
    public static func status(_ c: SourceCoverage, now: Date, timeZone: TimeZone = .current) -> String {
        let words = nouns(c.source)
        var parts: [String] = []
        if c.itemsRead == 0 { parts.append("nothing read yet") }
        else {
            parts.append("\(count(c.itemsRead)) \(words.item) read")
            if let newest = c.newestRead { parts.append("newest \(day(newest, timeZone))") }
        }
        if c.notRead > 0 { parts.append("\(count(c.notRead)) older not read") }
        parts.append("last looked \(ago(c.lastRun, now: now, timeZone: timeZone))")
        return parts.joined(separator: " · ")
    }
    /// "just now", "2 hours ago", "yesterday", "on 14 Sep" — when Brownie last asked this source for anything.
    static func ago(_ then: Date, now: Date, timeZone: TimeZone) -> String {
        let s = now.timeIntervalSince(then)
        if s < 0 { return "just now" }
        if s < 120 { return "just now" }
        if s < 3600 { return "\(Int(s / 60)) minutes ago" }
        if s < 86400 { let h = Int(s / 3600); return h == 1 ? "an hour ago" : "\(h) hours ago" }
        var cal = Calendar(identifier: .gregorian); cal.timeZone = timeZone
        let days = cal.dateComponents([.day], from: cal.startOfDay(for: then), to: cal.startOfDay(for: now)).day ?? 0
        if days == 1 { return "yesterday" }
        return "on \(day(then, timeZone))"
    }
